...
def run_1():
    command ="Rscript"
    path = 'C:\\Users\\kumud\\OneDrive\\Documents\\CBB_Project\\data_fromatting.R'
    cmd = [command,path]
...
def run_2():
    command ="Rscript"
    path = 'C:\\Users\\kumud\\OneDrive\\Documents\\CBB_Project\\bhawan.R'
    cmd = [command,path]
    x = subprocess.check_output(cmd,universal_newlines=True)
...
OptionList = ["BRCA(Breast Cancer)","PRAD(Prostate Cancer)","CESC(Cervical Cancer)","CHOL(BILE DUCT CANCER)","ESCA(Esophageal Cancer)","GBM(Glioblastoma)","KICH(Kidney ChromoPhobe)","THYM(Thymoma)"]
OptionList.sort()
